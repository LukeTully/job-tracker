export function titleWithinLength(title) {
    const min = 1;
    const max = 150;
...
export function validateSalaryMax(lowerRange, upperRange) {
    if (upperRange < lowerRange) {
        throw new Error(
            `The upper range of salary was less than the lower range (Upper: ${upperRange}, Lower: ${lowerRange})`,
        );
    }
    if (upperRange === lowerRange) {
        throw new Error(
            'The upper range of salary was the same as the lower range. If there is no lower range indicated, just set it to zero.',
        );
    }
    return true;
}
export function validateDescription(description) {
    ...
    // Ensure a range of characters
    if (descLen > maxLen) {
        throw new Error(`Description contained more than ${maxLen} characters`);
    }
    if (descLen < minLen) {
        throw new Error(`Description contained less than ${minLen} characters`);
    }
    return true;
}
export function validateExperienceLevel(level, availableLevels) {
    if (!availableLevels.includes(level)) {
        throw new Error(
            `The selected level (${level}) was not among the available seniority levels: ${availableLevels.join(
                ',',
            )}`,
        );
    }
...
export function validateInterviewCount(interviewCount) {
    ...
    if (interviewCount > maxInterviews) {
        throw new Error(
            `Interview count surpassed maximum of ${maxInterviews}: ${interviewCount}`,
        );
    }
    return true;
}
export function validateEasyApply(easyApply) {
...
export function validateRequiresPreInterviewTest(interviewTest) {
    if (!validateBoolean(interviewTest)) {
        throw new Error(
            'The setting for whether or not there\'s a pre interview test was not true/false',
        );
    }
    return true;
}
export function validateRequiresHomework(requiresHomework) {
    if (!validateBoolean(requiresHomework)) {
        throw new Error(
            'The setting for a job requiring a take home assignment was not true/false',
        );
    }
    return true;
}

export const requiresPreInterviewTest = 'The setting for whether or not there\'s a pre interview test was not true/false';
